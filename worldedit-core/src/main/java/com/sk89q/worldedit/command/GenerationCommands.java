...
public class GenerationCommands {
    ...
    @Command(
            name = "/volcano",
            desc = "Generates a volcano."
    )
    @CommandPermissions("worldedit.generation.git pull  gvolcano")
    @Logging(PLACEMENT)
    public int volcano(Actor actor, LocalSession session, EditSession editSession,
                    @Arg(desc = "The radii of the volcano. 1st is N/S, 2nd is E/W")
                    @Radii(2)
                    List<Double> radii,
                    @Arg(desc = "The height of the volcano", def = "1")
                    int height,
                    @Arg(desc = "Eruption of the volcano", def = "1")
                    boolean eruption
        ...
        double radiusZ;
        switch (radii.size()) {
            case 1 -> radiusX = radiusZ = Math.max(6, radii.get(0));
            case 2 -> {
                radiusX = Math.max(6, radii.get(0));
                radiusZ = Math.max(6, radii.get(1));
            }
            default -> {
                actor.printError(TranslatableComponent.of("worldedit.volcano.invalid-radius"));
                return 0;
            }
        }

        height = Math.max(6, height);
        worldEdit.checkvolcanoMaxRadius(radiusX);
        worldEdit.checkvolcanoMaxRadius(radiusZ);
        worldEdit.checkvolcanoMaxRadius(height);


        BlockVector3 pos = session.getPlacementPosition(actor);
        int affected = editSession.makeVolcano(pos, radiusX, radiusZ, height, eruption);
        actor.printInfo(TranslatableComponent.of("worldedit.volcano.created", TextComponent.of(affected)));
        return affected;
    }
