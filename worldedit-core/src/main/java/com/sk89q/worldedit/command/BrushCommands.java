...
public class BrushCommands {
    ...
    @Command(
            name = "climatechange",
            desc = "Climatechange brush, changes terrain to a desert, flood or snow"
    )
    @CommandPermissions("worldedit.brush.climatechange")
    void climateChangeBrush(Player player, LocalSession session,
                            @Arg(desc = "The size of the brush", def = "5")
                            double radius,
                            @Arg(desc = "Climate type: 'fire', 'flood', 'desert', or 'snow' or 'hell'", def = "")
                            String climateType,
                            @Arg(desc = "The intensity of the effect (1-10)", def = "5")
                            int intensity) throws WorldEditException {

        // Checks to see if the specified brush radius is within bounds
        worldEdit.checkMaxBrushRadius(radius);

        RegionFactory shape = new CuboidRegionFactory();


        // Different modes
        switch (climateType.toLowerCase()) {
            case "hot":
                hotMode(player, session, radius, intensity, shape);
                break;

            case "cold":
                coldMode(player, session, radius, intensity, shape);
                break;

            case "dry":
                dryMode(player, session, radius, intensity);
                break;

            case "wet":
                wetMode(player, session, radius, intensity, shape);
                break;
    ...
    private void hotMode(Player player, LocalSession session, double radius, int intensity, RegionFactory shape) throws WorldEditException {

        if (!isValidIntensity(intensity)) {
            player.printError(TranslatableComponent.of("worldedit.brush.climatechange.intensity"));
        } else {

            // Clears the weather in case it's raining
            player.getWorld().setWeather(WeatherType.REGISTRY.get("clear"));

            // Lowest intensity places fire
            if (intensity == 1) {
                setOperationBasedBrush(player, session, radius,
                        new Paint(new ReplaceFactory(BlockTypes.FIRE.getDefaultState()), Math.random()),
                        shape, "worldedit.brush.hot");
            } else if (intensity == 2) {
                setOperationBasedBrush(player, session, radius,
                        new Paint(new ReplaceFactory(BlockTypes.LAVA.getDefaultState()), Math.random()),
                        shape, "worldedit.brush.hot");
            } else if (intensity == 3) {
                // Sets feature to be used
                ConfiguredFeatureType deltaFeature = new ConfiguredFeatureType("delta");

                // Applies "delta" feature to terrain
                setOperationBasedBrush(player, session, radius,
                        new ApplyRegion(new FeatureGeneratorFactory(deltaFeature, 1)), shape, "worldedit.brush.feature");
            }
        }
    }

    private void coldMode(Player player, LocalSession session, double radius, int intensity, RegionFactory shape) throws WorldEditException {
        ...
        // Sets feature to be used
        ConfiguredFeatureType iceSpikeFeature = new ConfiguredFeatureType("ice_spike");

        int iceSpikeFactor = 0;
        if (intensity == 2) {
            iceSpikeFactor = 1000;
        } else if (intensity == 3) {
            iceSpikeFactor = 300;
        ...
        // Gives brush to the player
        BrushTool tool = session.forceBrush(
                player.getItemInHand(HandSide.MAIN_HAND).getType(),
                new FreezeBrush(1, 2, null, iceSpikeFeature, shape, intensity, iceSpikeFactor, session),
                "worldedit.brush.freeze"
        );
        tool.setSize(radius);
    }
}

    private void dryMode(Player player, LocalSession session, double radius, int intensity) throws WorldEditException {
        if (!isValidIntensity(intensity)) {
            player.printError(TranslatableComponent.of("worldedit.brush.climatechange.intensity"));
            ...
            BrushTool tool = session.forceBrush(
                    player.getItemInHand(HandSide.MAIN_HAND).getType(),
                    new DesertBrush(intensity),
                    "worldedit.brush.desert"
            );
            tool.setSize(radius);
    ...
    private void fillWater(Player player, LocalSession session, double radius, RegionFactory shape, double amount) throws WorldEditException {
        // Fills with water
        setOperationBasedBrush(player, session, radius,
                new Paint(new ReplaceFactory(BlockTypes.WATER.getDefaultState()), amount),
                shape, "worldedit.brush.flood");
    }


    private void wetMode(Player player, LocalSession session, double radius, int intensity, RegionFactory shape) throws WorldEditException {
        ...
        if (!isValidIntensity(intensity)) {
            ...
            // Rain duration (20 ticks = 1 second)
            int duration = (600);
            world.setWeather(WeatherType.REGISTRY.get("rain"), duration);

            if (intensity == 1) {
                fillWater(player, session, radius, shape, 0.5);
            } else if (intensity == 2) {
                fillWater(player, session, radius, shape, 1);
            } else if (intensity == 3) {
                fillWater(player, session, radius, shape, 1);
                world.setWeather(WeatherType.REGISTRY.get("thunder_storm"), duration);
            }
        }
    }
