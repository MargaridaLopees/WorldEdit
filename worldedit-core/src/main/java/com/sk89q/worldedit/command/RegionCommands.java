...
public class RegionCommands {
    ...
    public int line(Actor actor, EditSession editSession,
        ...
        if (!isValidRegion(region, actor, "worldedit.line.invalid-type")) { return 0; }

        // Checks if the thickness value is valid.
        checkValidParameter(thickness,"Thickness must be >= 0");

        // Retrieves the vertices of the selected region using the helper method.
        List<BlockVector3> vectors = getVertices(region);
    ...
    @Command(
            name = "/rift",
            desc = "Create a rift or valley-like or inverted as ridges structure in the selected region.",
            descFooter = "The rift method is responsible for creating a valley or crack or ridges in " +
                         "the selected region, with options to configure " +
                         "the thickness, height, block pattern, among others.")
    @CommandPermissions("worldedit.region.rift")
    @Logging(REGION)
    public int rift(Actor actor, EditSession editSession,
                    @Selection Region region,
                    @Arg(desc = "The thickness of the rift", def = "2")
                    double thickness,
                    @Arg(desc = "The height of the rift", def = "25")
                    double height,
                    @Arg(desc = "The wave amplitude of the rift", def = "5")
                    double waveAmplitude,
                    @Arg(desc = "The wave frequency of the rift", def = "1")
                    double waveFrequency,
                    @Arg(desc = "The pattern of blocks to place", def = "air")
                    Pattern pattern,
                    @Switch(name = 'u', desc = "Create upward rift") boolean up,
                    @Switch(name = 'd', desc = "Create downward rift") boolean down)
            throws WorldEditException {

        // Checks if the region is valid.
        if (!isValidRegion(region, actor, "worldedit.rift.invalid-type")) { return 0; }

        // Checks if the thickness and height values are valid.
        checkValidParameter(thickness,"Thickness must be >= 0");
        checkValidParameter(height, "Height must be >= 0");

        // Retrieves the vertices of the selected region using the helper method.
        List<BlockVector3> vectors = getVertices(region);

        int blocksChanged = 0;
        // If neither switch is specified, default to both
        if (!up && !down) {
            up = true;
            down = true;
        }

        // Create rifts based on switches
        if (up) {
            blocksChanged += editSession.drawValley(pattern, vectors, thickness, height, waveAmplitude, waveFrequency, false);
        }
        if (down) {
            blocksChanged += editSession.drawValley(pattern, vectors, thickness, height, waveAmplitude, waveFrequency, true);
        }

        actor.printInfo(TranslatableComponent.of("worldedit.rift.changed", TextComponent.of(blocksChanged)));
        return blocksChanged;
    }


    // Checks if the region is a convex polyhedral region.
    private boolean isValidRegion(Region region, Actor actor, String message) {
        if (!((region instanceof CuboidRegion) || (region instanceof ConvexPolyhedralRegion))) {
            actor.printError(TranslatableComponent.of(message));
            return false;
        } else
            return true;
    }

    // Checks if the value is valid (must be >= 0).
    private void checkValidParameter(double value, String errorMessage) throws WorldEditException {
        checkCommandArgument(value >= 0, errorMessage);
        WorldEdit.getInstance().checkMaxRadius(value);
    }

    // Retrieves the vertices of the selected region.
    private List<BlockVector3> getVertices(Region region) {

        List<BlockVector3> vectors;

        // If the region is a CuboidRegion, use its vertices. Otherwise, use the vertices of the ConvexPolyhedralRegion.
