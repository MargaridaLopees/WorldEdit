...
public class EditSession implements Extent, AutoCloseable {
    ...
    /**
     * Returns the type of block, decided randomly
     *
     * @param randomNumber random number calculated with math.random
     * @param frequency parameter used to do the % operation to do the random choice
     * @param type1 volcano's first block type option
     * @param type2 volcano's second block type option
     * @return type of block
     */
    private  BlockState randomBlock(int randomNumber,int frequency,BlockState type1, BlockState type2){
        BlockState block = type1;
        if(frequency!=0 && randomNumber % frequency==0)
            block = type2;
        return block;
    }
     ...
     * Returns the type of block, decided randomly
     *
     * @param currentHeight Y's current position to build the block
     * @param height volcano's height
     * @param eruption true if the volcano is in a eruptive state
     * @param random_Color random number calculated with math.random
     * @param ignoredLayers volcano's cone ignored layers, given the cone volcano height
     * @return type of block
     */
    private BlockState outsideConeBlock(int currentHeight, int height, boolean eruption, int random_Color, int ignoredLayers){
        BlockState block = null;
        if(eruption && currentHeight == height-ignoredLayers-1 ) {
            block = randomBlock(random_Color, 2, BlockTypes.LAVA.getDefaultState(), BlockTypes.MAGMA_BLOCK.getDefaultState());
        } else {
            block = randomBlock(random_Color, 2, block = BlockTypes.TERRACOTTA.getDefaultState(), BlockTypes.SOUL_SOIL.getDefaultState());
        } return block;
    }


    /**
     * Figures what type of block is supposed to be in a given position
     *
     * @param height volcano height
     * @param currentHeight Y's current position to build the block
     * @param ignoredLayers volcano's cone ignored layers, given the cone volcano height
     * @return type of block
     */
    private BlockState volcanoNonEruptionBlock(int height, int currentHeight, int ignoredLayers){
        BlockState block = null;
        if (currentHeight == height - ignoredLayers - 2)
            block=BlockTypes.HAY_BLOCK.getDefaultState();
        else if(currentHeight == height - ignoredLayers -3)
            block = BlockTypes.CAMPFIRE.getDefaultState();
        else  if(currentHeight < height -ignoredLayers )
            block = BlockTypes.LAVA.getDefaultState();
        return block;
    }

    /**
     * Checks if the current block's position is outside or inside the volcano's conduit
     *
     * @param posX X's position of the current block
     * @param posZ Z's position of the current block
     * @param radiusX The cone's largest north/south extent
     * @param radiusZ The cone's largest east/west extent
     * @return true is the block is inside the volcano's conduit area
     */
    private boolean insideVolcanoConduit(int posX, int posZ, double radiusX, double radiusZ){
        return (posX<=Math.round(radiusX * 0.1) && posZ<=Math.round(radiusZ * 0.1));
    }

    /**
     * Calculates the number of blocks changed to build volcano
     *
     * @param pos Center of the cone
     * @param radiusX The cone's largest north/south extent
     * @param block The type of the block to modify
     * @param distanceFromOriginMinusHeightSquared The calculation of distance from origin minus height squared
     * @return number of blocks changed
     */
    private int getAffected(BlockVector3 pos, int radiusX, BlockState block, double distanceFromOriginMinusHeightSquared, int x, int y, int z) throws MaxChangedBlocksException{
        int affected = 0;

        if (distanceFromOriginMinusHeightSquared <= 0) {
            if (setBlock(pos.add(x+radiusX , y, z), block)) {
                ++affected;
            }
            if (setBlock(pos.add(-x+radiusX, y, z), block)) {
                ++affected;
     ...
     * @throws MaxChangedBlocksException thrown if too many blocks are changed
     */
    public int makeVolcano(BlockVector3 pos, double radiusX, double radiusZ, int height
            ,boolean eruption) throws MaxChangedBlocksException {
        int affected = 0;
        int ignoredLayers = (int) Math.ceil(0.3*height);
        height += ignoredLayers;
        BlockState block = BlockTypes.BROWN_TERRACOTTA.getDefaultState();
        for (int y = 0; y < height; ++y) {
            double ySquaredMinusHeightOverHeightSquared = Math.pow(y - height, 2) / Math.pow(height, 2);
            ...
            for (int x = 0; x <= (int) Math.ceil(radiusX); ++x) {
                ...
                for (int z = 0; z <= (int) Math.ceil(radiusZ); ++z) {
                    ...
                    int random_Color = (int) (Math.random() * 100);

                    //ignore the first 4 levels
                    if (y >= height - ignoredLayers) continue; //&& !eruption

                    if (insideVolcanoConduit(x, z, radiusX, radiusZ)) {
                        //inside the cone
                        if (!eruption)
                            block = volcanoNonEruptionBlock(height, y, ignoredLayers);
                        else block = BlockTypes.LAVA.getDefaultState();
                    } else  //cone
                        block = outsideConeBlock(y, height, eruption, random_Color, ignoredLayers);
                    ...
                    affected += getAffected(pos, (int) radiusX, block, distanceFromOriginMinusHeightSquared, x,y,z);
                    }
                }
            }
            return affected;
    ...
    */
    ...
    /**
     * Draws a line (out of blocks) between two or more vectors in the world.
     * The line can have a specified thickness (radius) and can be either solid or hollow.
     *
     * @param pattern The block pattern used to draw the line.
     * @param vectors A list of points (vectors) through which the line is drawn.
     * @param radius  The radius (thickness) of the line.
     * @param filled  If true, the line will be solid (filled). Else, the line will be hollow (a shell).
     *
     * @return The number of blocks affected in the world.
     * @throws MaxChangedBlocksException Thrown if the operation attempts to modify too many blocks,
     *                                    exceeding a predefined limit for safety reasons.
     */
    public int drawLine(Pattern pattern, List<BlockVector3> vectors, double radius, boolean filled)
            throws MaxChangedBlocksException {

        // Generate the intermediate points along the line between the vectors.
        Set<BlockVector3> vset = calculateIntermediatePoints(vectors, null);

        // Apply a "ballooning" transformation to increase the line's thickness.
        vset = getBallooned(vset, radius);

        // If the line is not supposed to be filled, make it hollow.
        if (!filled) { vset = getHollowed(vset); }

        // Place the calculated set of blocks into the world using the specified pattern.
        return setBlocks(vset, pattern);
    }


    /**
     * Draws a valley (a sinusoidal-shaped depression) between two or more vectors in the world.
     *
     * @param pattern       The block pattern used to draw the valley.
     * @param vectors       A list of points (vectors) through which the valley is drawn.
     * @param radius        The radius (thickness) of the valley.
     * @param height        The height of the valley.
     * @param waveAmplitude The amplitude of the sinusoidal wave.
     * @param waveFrequency The frequency of the sinusoidal wave.
     * @param inv           If true, the valley is inverted (protrudes upward). Else,the valley forms a depression.
     *
     * @return The number of blocks affected in the world.
     * @throws MaxChangedBlocksException Thrown if the operation attempts to modify too many blocks,
     *                                    exceeding a predefined limit for safety reasons.
     */
    public int drawValley(Pattern pattern, List<BlockVector3> vectors, double radius, double height,
                          double waveAmplitude, double waveFrequency, boolean inv)
            throws MaxChangedBlocksException {

        // Generate intermediate points along the path, applying sinusoidal wave deformations.
        Set<BlockVector3> vset = calculateIntermediatePoints(vectors, (tip, progress) -> {
            // Apply a sinusoidal wave offset to the X coordinate based on the current progress.
            // The wave offset is calculated using the sine function, with the specified amplitude and frequency.
            int waveOffset = (int) (Math.sin(progress * Math.PI * 2 * waveFrequency) * waveAmplitude);

            // Return a new BlockVector3 with the wave offset applied to the X coordinate.
        ...
        // Transform the set of blocks into an ellipsoid shape to simulate the valley's profile.
        vset = getEllipsoid(vset, radius, height, inv);
    ...
    /**
     * Calculates the intermediate points along a path defined by a list of vectors.
     *
     * @param vectors        A list of points (vectors) defining the path.
     * @param transformation An optional transformation applied to each intermediate point.
     *
     * @return A set of all intermediate points, including the start and end points of each segment.
     */
    private Set<BlockVector3> calculateIntermediatePoints(List<BlockVector3> vectors,
                                                          BiFunction<BlockVector3, Double, BlockVector3> transformation) {
        // Set to store the calculated intermediate points.
        Set<BlockVector3> vset = new HashSet<>();

        // Iterate through each pair of consecutive vectors in the list.
        for (int i = 0; !vectors.isEmpty() && i < vectors.size() - 1; i++) {
            BlockVector3 pos1 = vectors.get(i);      // Starting point of the segment.
            BlockVector3 pos2 = vectors.get(i + 1);  // Ending point of the segment.
            ...
            // If the points are identical, add the starting point to the set and skip.
            if (dx + dy + dz == 0) {
                vset.add(pos1);
                //continue;
            }

            // Determine the dominant axis (the one with the largest difference).
            int dMax = Math.max(Math.max(dx, dy), dz);

            // Loop through the range [0, dMax], interpolating points along the segment.
            for (int domstep = 0; domstep <= dMax; domstep++) {
                // Calculate the progress (normalized value between 0.0 and 1.0).
                double progress = (double) domstep / dMax;

                // Interpolate the coordinates based on the progress.
                int tipx = (int) Math.round(x1 + progress * (x2 - x1));
                int tipy = (int) Math.round(y1 + progress * (y2 - y1));
                int tipz = (int) Math.round(z1 + progress * (z2 - z1));

                // Create a new point at the interpolated coordinates.
                BlockVector3 tip = BlockVector3.at(tipx, tipy, tipz);

                // Apply the optional transformation, if provided.
                if (transformation != null) {
                    tip = transformation.apply(tip, progress);
                }

                // Add the point to the set of intermediate points.
                vset.add(tip);
            }
        }
        // Return the set of all calculated intermediate points.
        return vset;
    }
    ...
    private static Set<BlockVector3> getEllipsoid(Set<BlockVector3> vset, double radius, double height, boolean inverted) {
        Set<BlockVector3> returnset = new HashSet<>();
        int ceilrad = (int) Math.ceil(radius);
        int ceilhei = (int) Math.ceil(height);
        ...
        for (BlockVector3 v : vset) {
            ...
            int minY = inverted ? tipy - ceilhei : tipy;
            int maxY = inverted ? tipy : tipy + ceilhei;

            for (int loopx = tipx - ceilrad; loopx <= tipx + ceilrad; loopx++) {
                for (int loopy = minY; loopy <= maxY; loopy++) {
                    for (int loopz = tipz - ceilrad; loopz <= tipz + ceilrad; loopz++) {
                        // Calculate normalized squared distances
                        double dx = (loopx - tipx) / radius;
                        double dy = (loopy - tipy) / height;
                        double dz = (loopz - tipz) / radius;
